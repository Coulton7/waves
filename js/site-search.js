    ...
    search.on('render', function() {
        ...
        for (var x = 0; x < refineType.length; x++) {
            ...
            if(refineType[x].innerText.includes('article')) {
                refineType[x].innerText = refineType[x].innerText.replace('article', '<input type="checkbox" class="ais-refinement-list--checkbox" value="Article" checked="true"/> Article');
                ...
                refineType[x].innerText = refineType[x].textContent.replace("locations","Locations");
            }
        }
